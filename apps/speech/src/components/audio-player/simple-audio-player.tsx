...
import { formatTimeInSeconds } from "@/utils/time";
import {
  ChevronDownIcon,
  FastForwardIcon,
  PauseIcon,
...
const PlayButtonIcon = ({
  isLoading,
  isPlayingState,
}: { isLoading: boolean; isPlayingState: boolean }) => {
  if (isLoading) {
    return (
      <motion.div
        key="loading"
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.15 }}
    >
      <PlayIcon className="w-5 h-5 translate-x-[5%]" />
    </motion.div>
  );
};
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === " ") {
        ...
        e.preventDefault();
        if (audioRef.get()) {
          audioRef.get()!.currentTime = Math.max(
            0,
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 bg-background border-t border-gray-200 shadow-lg"
      >
        ...
        <div className="flex items-center gap-4 px-4 py-3">
          ...
          <Button
            onClick={toggleAudio}
            size="icon"
            className="w-12 h-12 rounded-full bg-black dark:bg-white text-white dark:text-black hover:scale-105 transition-transform"
            aria-label={isPlaying.get() ? "Pause" : "Play"}
          >
            <AnimatePresence>
              <PlayButtonIcon
                isLoading={isLoading}
                isPlayingState={isPlaying.get()}
              />
            </AnimatePresence>
          </Button>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <h3 className="font-medium text-sm truncate">
                  {currentAudio.get()!.title}
                </h3>
                {currentAudio.get()!.description && (
                  <p className="text-xs text-[#878787] truncate hidden md:block">
                    {currentAudio.get()!.description}
                  </p>
                )}
              </div>
