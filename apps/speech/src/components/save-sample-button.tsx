...
type SaveSampleFormSchema = z.infer<typeof SaveSampleSchema>;
...
export function SaveSampleButton({
  ...
  const form = useForm<SaveSampleFormSchema>({
    resolver: zodResolver(SaveSampleSchema),
    defaultValues: {
      fileName: slug,
    },
  });
  ...
  async function onSubmit({ fileName }: SaveSampleFormSchema) {
    try {
